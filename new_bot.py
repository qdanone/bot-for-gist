...
boss_ids = [int(x) for x in os.getenv("BOSS_IDS").split(",")]
worker_ids = [int(x) for x in os.getenv("WORKER_IDS").split(",")]

users_info_raw = os.getenv("USERS_INFO")
users_info = {int(k): v for k, v in (x.split(":") for x in users_info_raw.split(","))}
...
creds = ServiceAccountCredentials.from_json_keyfile_name(
    "telegram-bot-489810-4d74ec45f2bb.json",
    scope
)

client = gspread.authorize(creds)

# основной лист с препаратами
...
def load_data():
    ...
    if "Порядковый номер" in df.columns:
        df["Порядковый номер"] = pd.to_numeric(
            df["Порядковый номер"],
            errors="coerce"
        ).fillna(0).astype(int)
...
async def add_name(message: Message, state: FSMContext):
    ...
    if existing.empty:
        ...
        next_number = existing["Порядковый номер"].max() + 1
...
async def search_name(message: Message, state: FSMContext):
    ...
    result = df[
        df["Препарат"].str.lower().str.contains(query, na=False)
    ]
